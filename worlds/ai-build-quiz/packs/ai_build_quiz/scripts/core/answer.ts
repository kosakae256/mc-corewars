 ...
 *
 * **bridge の `src/normalize.ts` と同じ内容にしておくこと。** 片方だけ変えると
 * bridge が作った `accepted` と噛み合わず「正解なのに不正解」になる。
 * 同じ検証ベクタ（`tests/core.test.ts` と bridge の `src/tools/test.ts`）を両方で通す。
 *
 * > ### 方針: 意味が伝わればよい（本人・2026-09-21）
 * >
 * > 厳しくして外すより、緩くして当てる。語尾（「犬かな」）・打ち間違い・部分一致を通す。
 */
