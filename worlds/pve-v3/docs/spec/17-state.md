...
| | |
| --- | --- |
| **入口でやること** | **全部リセット**——参加を解く／強化とエメラルドを消す／HPコマンド指定を消し、**現在HP・最大HPを100/100へ戻す**／呪いと wave を消す／**戦場の敵を片付ける**／全員をロビーへ |
...
2026-09-11: 全滅後にHP0のまま戻る不具合を修正。強化を消した後にHPの指定値と指定時の強化基準も消し、
初期HP100で満タンにする。周期の最大HP反映で古い指定値へ戻らないようにする。
処理はロビー移動より先に完了し、クリエイティブにも適用する。
既存の不整合（idleなのに参加中・死亡フラグあり・現在HP0）はpresenceの周期処理でも同じリセットで修復する。
試合中の死亡者や、ロビーでHPが残っている人はこの補修で回復させない。
...
### 3-4. **ブロックに埋まったら、湧く所へ戻す**（2026-09-06 追加）

> ### マップの差し替えに追いつかれて、埋まることがある
>
> **差し替えは `runJob` で少しずつ進む。**
> 運ばれた先が**まだ前のマップのまま**だったり、
> **置かれた地形の中に入ってしまう**ことがある。
> 埋まると**動けず、抜け出せない。**

| | |
| --- | --- |
| **いつ見るか** | **wave の間だけ。** スペクテイターは見ない（すり抜けられる） |
| **埋まっている条件** | **足元と頭の両方**が、空気でも液体でもない**実体のあるブロック**（`21-spawn-mark.md` の判定と同じ） |
| **どれだけ続いたら** | **1 秒（20 tick）。** 一瞬の行き違いでは戻さない |
| **どこへ戻すか** | **そのマップの湧く所**（`PLACES.field`）。向きも ＋z に直す |
| **知らせ方** | **何も出さない**（2026-09-06 決定）。戦っている最中に読ませるものではない |

**1 秒待つのは、地形が置かれる途中の一瞬を拾わないため。**
戻した後もまだ埋まっていれば、**また 1 秒後に戻す**（湧く所は必ず空いている——`14-map-build.md` 0-1）。

### 3-5. **奈落に落ちたら、湧く所へ戻す**（2026-09-06 追加）

**マップの多くは浮島で、縁の外は奈落**（`14-map-build.md` 0-4）。
**落ちても死なない。戻す。ただし、ただでは戻さない。**

| | |
| --- | --- |
| **いつ見るか** | **wave の間だけ。** スペクテイターは見ない |
| **落ちたと認める高さ** | **y ≤ −30。** 戦場の底（y −50）より上に置いて、**落ち切る前に戻す** |
| **戻す先** | **そのマップの湧く所**（`PLACES.field`）。向きも ＋z |
| **代償** | **いまの HP の 25%** を削る（切り捨て） |
| **削らない場合** | **HP が 3 以下のとき。** 奈落で倒れることはない |
...
**削るのは `services/combat.ts` の `hit()`**——通り道は 1 本（`12-architecture.md` 2-3）。
**赤く光る・音が鳴る**のも、そこで一緒に付く。

#### 敵も同じ高さで拾う（2026-09-08 追加）

**敵が落ちても、そのまま消えたりはしない。** **プレイヤーと同じ条件で戻す。**

| | プレイヤー | **敵** |
| --- | --- | --- |
| **落ちたと認める高さ** | y ≤ −30 | **同じ** |
| **戻す先** | そのマップの湧く所（`PLACES.field`） | **敵の湧き点**（`21-spawn-mark.md`）**からランダムに 1 つ** |
| **HP** | **いまの 25 % を削る** | **削らない。そのまま** |
| **向き** | ＋z に向ける | 触らない |

> ### **半径 3 マスの決まりは守る**
>
> **湧かせるときと同じ**——**プレイヤーの半径 3 マス以内には出さない**（`16-enemy.md` 3-4-1）。
> **目の前に湧き直すのは、避けようがない。**

**湧き点が 1 つも無いマップでは、戻さない**（そのまま落ちたままにする）。
**ウェーブ終わりの後片付けで消える。**

### 3-7. **炎と溶岩は削る**（2026-09-08 追加）

**バニラのダメージは全部打ち消している**（`11-damage.md` 1 章）**が、炎だけは置き換える。**
**燃えているのに何も起きないのは、見た目と噛み合わない。**

| | |
| --- | --- |
| **拾う原因** | **`fire`（火の中）／ `fireTick`（燃えている）／ `lava`（溶岩）** |
| **削る量** | **最大 HP の 2 %**（切り捨て・**最低 1**） |
| **いつ** | **バニラが 1 回削るたびに 1 回**——**燃えている間は 0.5 秒ごと** |
| **重ねない** | **同じ実体は 8 tick に 1 回まで** |

> ### **炎は原因が 2 つ同時に来る**（2026-09-08 に気づいた）
>
> **火の中に立つと `fire` が、燃えていると `fireTick` が、同じ拍で飛んでくる。**
> **溶岩も `lava` ＋ `fireTick`。**
> **そのまま拾うと 2 回削れて、音も 2 回鳴った。** **8 tick の間隔で 1 回にまとめる。**

> ### **割合にする理由**
>
> **固定値だと、育てた人ほど痛くなくなる。**
> **最大 HP の 2 % なら、誰にとっても同じ重さ**（`3-5` の奈落と同じ考え方）。

**削るのは `services/combat.ts` の `hit()`**——**赤く光る・音が鳴る**のも一緒に付く。
**`beforeEvents` の中では削れない**ので、**次の tick に回している**（`events/hurt.ts`）。

**敵モブは燃えない**——**`minecraft:fire_immune` を持たせてある**（`24-mob-howto.md` 9 章）。

### 3-6. **満腹度は常に満タン**（2026-09-07 決定）

> ### 空腹はこのゲームの遊びではない
>
> **食べ物を探す・走れなくなる・じわじわ減る**——**どれも要らない。**

| | どこ |
| --- | --- |
| **減らさない** | `entities/player.json` の `minecraft:exhaustion_values` を**全部 0** に |
| **満タンで始める** | 同・`minecraft:player.saturation` を **20** に |
| **戻す** | **減った状態から戻らない**ので、`features/mob` が**毎周期満タンに戻す** |

**2 段構えにしている**——**入り直した人・設定を変える前から居た人**も戻せるように。
