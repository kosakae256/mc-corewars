 ...
 * | 戦場 | **マップごとに `x ＝ 1000 × 番号`**（2026-09-07 変更） |
 *
 * **足元の y は 1 つ上に置く**——中心 y のブロックに埋まらないように。
 *
 * > ### **戦場の座標は、ここでは決まらない**
 * >
 * > **マップは 1000 マスずつ離して常設してある**（`spec/19-map-store.md` 0 章）。
 * > **ここに書いてある戦場の値は「マップの原点からの相対」。**
 * > **絶対の座標が要るときは `services/arena.ts` を通す。**
 * >
 * > **`PLACES.field` は作業台（x 0）**——**マップを作る所**であって、試合の場ではない。
 */
/**
...
export const PLACES = {
  ...
  // **作業台の湧く所**（x 0）。**試合で使うのは `services/arena.ts` の `spawnSpot()`**
