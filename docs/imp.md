...
### 10-10. パックを直したら、manifest の版を上げる（2026-09-05 決定）

> ### 版を上げないと、直したものが届かないことがある
>
> **Minecraft はパックを版で見分ける。**
> 同じ版のまま中身だけ変えると、**古いものが使われ続ける**ことがある——
> ワールドに焼き付いた参照、他の端末、配布した `.mcaddon`。
> **絵やモデルを差し替えたのに変わらない**とき、まずここを疑う。

**リソースパック・ビヘイビアーパックのどちらでも、中身を直したら版を上げる。**

| どこ | いつ上げるか |
| --- | --- |
| `header.version` | **中身を直したら毎回**（末尾を ＋1） |
| `modules[].version` | 同上。**header と揃える** |
| `dependencies[].version` | **相手の版を上げたら、こちらも直す**（BP → RP の参照） |

```bash
node tools/pack-bump.mjs worlds/<名>/packs/<パック>
```

**手で 3 か所を直すと必ずどれか忘れる**ので、道具を通す。
