...
system.beforeEvents.startup.subscribe((init) => {
  ...
  // init.blockComponentRegistry.registerCustomComponent("__ADDON__:example", {
  //   onStepOn: (e, params) => { /* ... */ },
  // });
  //
  // 例: アイテム用 Custom Component の登録
  // init.itemComponentRegistry.registerCustomComponent("__ADDON__:example", {
  //   onUse: (e, params) => { /* ... */ },
  // });
  void init;
});
...
world.afterEvents.worldLoad.subscribe(() => {
  world.sendMessage("§a__ADDON__§r loaded.");
  main();
});

function main(): void {
  // 5秒（100 tick）ごとに動作確認メッセージを出す
  system.runInterval(() => {
    world.sendMessage(`tick: ${system.currentTick}`);
  }, 100);
}
